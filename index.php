...
margin: 2% auto 10% auto; /* 5% from the top, 15% from the bottom and centered */ border: 1px solid #888;
...
.big-btn { width: 180px; height: 40px;
} </style>
<?php include("header12.php");
include("dbconnect.php"); extract($_POST);
if(isset($submit)) {
$rs=mysql_query("select * from users where username='$username' and password= MD5('$password')"); if(mysql_num_rows($rs)<1)
{
$found="N";
$_SESSION[login]=$username; header("Location: welcome.php");
} else {
} }
?>
</head>
...
<span onclick="document.getElementById('id01').style.display='none'" class="close" title="Close Modal">&times;</span> <img src="admin.png" alt="Avatar" class="avatar">
</div>
<div class="container">
...
<button type="submit" name="submit" id="submit" value="Login">Login</button> <!-- <label>
...
<form class="modal-content animate" method="post" action=""> <div class="imgcontainer">
...
<script>
// Get the modal
var modal = document.getElementById('id01'); var modal = document.getElementById('id02');
